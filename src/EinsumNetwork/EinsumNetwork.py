...
from src.EinsumNetwork.FactorizedLeafLayer import FactorizedLeafLayer
from src.EinsumNetwork.SumLayer import SumLayer
from src.EinsumNetwork.EinsumLayer import EinsumLayer
from src.EinsumNetwork.EinsumMixingLayer import EinsumMixingLayer
from src.EinsumNetwork.graph.helper_functions import check_graph, get_roots, get_leaves, get_sums, topological_layers
...
class EinsumNetwork(torch.nn.Module):
    ...
    def _backtrack_EinsumLayer(self, x, layer, dist_idx, reg_idx, sample_idx, mode, kwargs):
        ...
        return sample_idx, dist_idx, reg_idx

    def _backtrack_EinsumMixingLayer(self, x, layer, dist_idx, reg_idx, sample_idx, mode, kwargs):
        ret = layer.backtrack(dist_idx[layer],
                                reg_idx[layer],
                                sample_idx[layer],
                                use_evidence=(x is not None),
    ...
    def _backtrack_FactorizedLeafLayer(self, root, x, layer, dist_idx, reg_idx, sample_idx, mode, kwargs):
    ...
    def backtrack(self, num_samples=1, class_idx=0, x=None, mode='sampling', **kwargs):
        ...
        for layer in reversed(self.einet_layers):
            if not sample_idx[layer]:
                continue
            args = (x, layer, dist_idx, reg_idx, sample_idx, mode, kwargs)
            if type(layer) == EinsumLayer:
                out = self._backtrack_EinsumLayer(*args)
                sample_idx, dist_idx, reg_idx = out
            elif type(layer) == EinsumMixingLayer:
                out = self._backtrack_EinsumMixingLayer(*args)
                sample_idx, dist_idx, reg_idx = out
            elif type(layer) == FactorizedLeafLayer:
                return self._backtrack_FactorizedLeafLayer(root, *args)

    def sample(self, num_samples=1, class_idx=0, x=None, **kwargs):
        samples = self.backtrack(num_samples=num_samples, class_idx=class_idx, x=x, mode='sample', **kwargs)
        if len(samples.shape) == 2:
            samples = samples.unsqueeze(2)

        N, H_W, C = samples.shape
        H = int(math.sqrt(H_W))
        # print("Warning! Assuming a square image")
        return samples.view(N, H, H, C).permute(0, 3, 1, 2)
...
def eval_loglikelihood_batched2(einet, x, labels=None, batch_size=100):
    """Computes log-likelihood in batched way."""
    with torch.no_grad():
        lls = torch.empty(x.shape[0], device=x.device)
        idx_batches = torch.arange(0, x.shape[0], dtype=torch.int64, device=x.device).split(batch_size)
        for batch_count, idx in enumerate(idx_batches):
            batch_x = x[idx, :]
            if labels is not None:
